package com.shanebeestudios.skassist.elements.bound.events;
...
import com.shanebeestudios.skassist.api.event.bound.BoundEnterEvent;
import com.shanebeestudios.skassist.api.event.bound.BoundEvent;
import com.shanebeestudios.skassist.api.event.bound.BoundExitEvent;
import com.shanebeestudios.skassist.api.listener.BoundBorderListener;
...
public class BoundEvents extends SkriptEvent {
    ...
    static {
        ...
        Skript.registerEvent("Bound - Enter", BoundEvents.class, BoundEnterEvent.class, "(bound enter|enter bound) [with id %-string%]")
            .description("Called when a player enters a bound. Optional ID of bound. 'event-string' = bound ID.",
                "NOTE: Due to breaking changes in Bukkit API, enter/exit events will not be called when a player mounts/dismounts an entity if running SkAssist 3.5.0+ on MC 1.20.4 and below.")
    ...
    public boolean init(Literal<?>[] args, int matchedPattern, ParseResult parseResult) {
        ...
        if (boundBorderListener == null) {
            ...
            SkAssist plugin = SkAssist.getPlugin();
